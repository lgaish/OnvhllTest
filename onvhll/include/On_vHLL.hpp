...
public:
  ...
  On_vHLL(std::uint32_t stage_num, std::uint32_t row, std::uint32_t col,
          std::uint32_t master_seed)
      : op_statistics(
            "../../result/operation_statistics/on_vhll_op_statistics.txt") {
  ...
  bool update(const char *flow_id, uint64_t flow_id_len, const char *element_id,
              uint64_t element_id_len) {
    bool FLAG_UPDATED = false;
    for (auto &stage : stages) {
      std::uint32_t select_row =
          stage->selectRowIdxByFlowID(flow_id, flow_id_len);
      std::uint32_t select_col = stage->selectColIdxByElementID(
          flow_id, flow_id_len, element_id, element_id_len);
      std::uint8_t leading_zeroes = stage->countLeaderZero(
          flow_id, flow_id_len, element_id, element_id_len);
  ...
  double get_estimate(const char *flow_id, uint64_t flow_id_len) {
  ...
  void getHistogram(int true_card) {
    ...
    std::cout << "sum:" << n_hat_1 << ",dev:" << n_hat_1 - true_card
              << ",r_dev:" << (n_hat_1 - true_card) / true_card;
    std::cout << "sum:" << n_hat_2 << ",dev:" << n_hat_2 - true_card
              << ",r_dev:" << (n_hat_2 - true_card) / true_card;
