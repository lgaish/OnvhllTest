...
private:
  ...
  static inline double derivation_of_Pr_Mj_eq_k(int k, int num_of_estimators, double estimation) {
    ...
    double a = 1 - 1.0 / ((long long)num_of_estimators * (long long)(1 << k));
    double b = 1 - 1.0 / ((long long)num_of_estimators * (long long)(1 << (k - 1)));
    double ret = std::pow(a, estimation) * std::log2(a) - std::pow(b, estimation) * std::log2(b);
    return ret;
  }

  static double derivation_of_log_L(double estimation, std::array<uint32_t, HISTOGRAM_LEN> histogram, uint32_t histogram_len,
                                    int num_of_estimators) {
    double ret = 0;
...
public:
  ...
  Histogram(std::uint32_t reg_num, bool lc_mle_enabler = true) : lc_mle_enabler(lc_mle_enabler) {
