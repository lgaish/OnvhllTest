...
inline void override_seed(uint64_t s) { rand.seed(s); }
} // namespace random_utils
...
template <typename T> static inline T read(std::istream &is) {
  T value;
...
template <typename T> static inline void write(std::ostream &os, T value) {
...
template <typename T> static inline T read_big_endian(std::istream &is) {
  T value;
  is.read(reinterpret_cast<char *>(&value), sizeof(T));
...
template <typename T> class return_value_holder {
public:
